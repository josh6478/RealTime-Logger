...
**RT Logger** enables remote debug and telemetry prints (DIM = Debug Information Message) from IoT device to a remote server. It supports on-device buffering (Pull) and live streaming (Push), selective verbosity by module and level, and simultaneous mirroring to UART.
